...
import { Eye, EyeOff, Phone, Mail, ArrowRight, Wrench } from 'lucide-react'
...
export default function WorkerLogin() {
  ...
  const handleGoogleSignIn = async () => {
    try {
      const { error } = await supabase.auth.signInWithOAuth({ provider: 'google', options: { redirectTo: `${window.location.origin}/dashboard/worker` } })
      if (error) throw error
  ...
  const ic = (f) => `w-full px-4 py-2.5 rounded-lg bg-gray-800 border ${errors[f] ? 'border-red-500' : 'border-gray-700'} text-white placeholder-gray-400 focus:outline-none focus:border-green-500 transition`
  ...
  return (
    <div className="min-h-screen bg-[#0b1120] flex items-center justify-center px-4 relative overflow-hidden">
      <div className="absolute top-20 right-20 w-80 h-80 bg-green-500/5 rounded-full blur-3xl pointer-events-none" />
      ...
      <div className="w-full max-w-md relative z-10">
        <div className="bg-slate-800/60 backdrop-blur-sm border border-white/10 rounded-2xl shadow-2xl p-8">
          <div className="text-center mb-8">
            ...
            <h1 className="text-3xl font-bold text-white">Worker Sign In</h1>
            <p className="text-slate-400 text-sm mt-1">Welcome back! Manage your jobs & grow your business</p>
          </div>
          ...
          <div className="flex rounded-lg overflow-hidden border border-gray-700 mb-6">
            <button type="button" onClick={() => { setLoginMode('email'); setOtpSent(false); setOtpError('') }}
              className={`flex-1 py-2.5 text-sm font-medium transition flex items-center justify-center gap-2 ${loginMode === 'email' ? 'bg-green-500 text-white' : 'bg-gray-800 text-gray-400 hover:text-white'}`}>
              <Mail size={15} /> Email
            </button>
            <button type="button" onClick={() => { setLoginMode('phone'); setOtpError('') }}
              className={`flex-1 py-2.5 text-sm font-medium transition flex items-center justify-center gap-2 ${loginMode === 'phone' ? 'bg-green-500 text-white' : 'bg-gray-800 text-gray-400 hover:text-white'}`}>
          ...
          {loginMode === 'email' && (
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="flex justify-end">
                <Link to="/forgot-password" className="text-green-400 hover:text-green-300 text-xs font-medium transition">Forgot Password?</Link>
              </div>
              <button type="submit" disabled={loading}
                className="w-full py-3 bg-gradient-to-r from-green-500 to-emerald-600 hover:from-green-400 hover:to-emerald-500 disabled:opacity-50 disabled:cursor-not-allowed text-white font-semibold rounded-lg transition-all duration-300 mt-2 shadow-lg shadow-green-500/20">
            ...
            <div className="space-y-4">
              <div>
                ...
                <div className="flex">
                  ...
                  <input type="tel" maxLength={10} value={phone} onChange={(e) => { setPhone(e.target.value.replace(/\D/g, '')); setOtpError('') }}
                    ...
                    className={`flex-1 px-4 py-2.5 rounded-r-lg bg-gray-800 border border-gray-700 text-white placeholder-gray-400 focus:outline-none focus:border-green-500 transition ${otpSent ? 'opacity-60' : ''}`} />
                </div>
              </div>
              {otpSent && (
                <div>
                  ...
                  <input type="text" maxLength={6} value={otp} onChange={(e) => { setOtp(e.target.value.replace(/\D/g, '')); setOtpError('') }}
                    placeholder="6-digit code" className="w-full px-4 py-2.5 rounded-lg bg-gray-800 border border-gray-700 text-white placeholder-gray-400 focus:outline-none focus:border-green-500 transition text-center tracking-[0.5em] text-lg font-mono" autoFocus />
                </div>
              )}
              ...
              {!otpSent ? (
                <button type="button" onClick={handleSendOtp} disabled={otpLoading || phone.length < 10}
                  className="w-full py-3 bg-gradient-to-r from-green-500 to-emerald-600 hover:from-green-400 hover:to-emerald-500 disabled:opacity-50 disabled:cursor-not-allowed text-white font-semibold rounded-lg transition-all duration-300 flex items-center justify-center gap-2 shadow-lg shadow-green-500/20">
          ...
          <div className="mt-6 space-y-2 text-center">
            <p className="text-gray-400 text-sm">Don't have an account?{' '}<Link to="/register/worker" className="text-green-400 hover:text-green-300 font-medium">Register as Worker</Link></p>
            <p className="text-gray-500 text-xs">Looking for services?{' '}<Link to="/login/customer" className="text-blue-400 hover:text-blue-300 font-medium">Sign in as Customer →</Link></p>
          </div>
        </div>
      </div>
    </div>
