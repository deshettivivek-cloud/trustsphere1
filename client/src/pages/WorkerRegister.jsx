...
export default function WorkerRegister() {
  ...
  const validate = () => {
    ...
    if (!form.service_type) err.service_type = "Service type required"
    return err
  }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const { data, error: signUpError } = await supabase.auth.signUp({
        ...
        options: { data: { full_name: form.full_name, phone: form.phone, city: form.city, role: "service_worker" } },
      })
      if (signUpError) throw signUpError
      if (!data.user) throw new Error("User creation failed")

      const { error: profileError } = await supabase.from("profiles").upsert({
        id: data.user.id, full_name: form.full_name, phone: form.phone, city: form.city, role: "service_worker", email: form.email,
      })
      if (profileError) throw profileError

      // Insert worker_profiles
      const { data: existingWp } = await supabase.from("worker_profiles").select("id").eq("user_id", data.user.id).maybeSingle()
      if (existingWp) {
        await supabase.from("worker_profiles").update({ service_type: form.service_type, location: form.city }).eq("user_id", data.user.id)
      } else {
        await supabase.from("worker_profiles").insert({ user_id: data.user.id, service_type: form.service_type, location: form.city })
      }
  ...
  return (
    <div className="min-h-screen bg-[#0b1120] flex items-center justify-center px-4 py-8 relative overflow-hidden">
      <div className="absolute top-20 left-20 w-80 h-80 bg-green-500/5 rounded-full blur-3xl pointer-events-none" />
      ...
      <div className="w-full max-w-md relative z-10">
        <div className="bg-slate-800/60 backdrop-blur-sm border border-white/10 rounded-2xl shadow-2xl p-8">
          <div className="mb-6 text-center">
            ...
            <h2 className="text-3xl font-bold text-white">Create Worker Account</h2>
            <p className="text-slate-400 mt-1 text-sm">Join InLocFix and grow your service business</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <div className="flex">
                ...
                <input type="tel" placeholder="Phone Number" value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })}
                  className={`flex-1 px-4 py-2.5 rounded-r-lg bg-gray-800 border ${errors.phone ? "border-red-500" : "border-gray-700"} text-white placeholder-gray-400 focus:outline-none focus:border-green-500 transition`} />
              </div>
            ...
            <div>
              <select value={form.service_type} onChange={(e) => setForm({ ...form, service_type: e.target.value })}
                className={`w-full px-4 py-2.5 rounded-lg bg-gray-800 border ${errors.service_type ? "border-red-500" : "border-gray-700"} text-white focus:outline-none focus:border-green-500 transition`}>
          ...
          <div className="mt-6 space-y-2 text-center">
            <p className="text-gray-400 text-sm">Already have an account?{" "}<Link to="/login/worker" className="text-green-400 hover:text-green-300 font-medium">Sign in</Link></p>
            <p className="text-gray-500 text-xs">Looking for services?{" "}<Link to="/register/customer" className="text-blue-400 hover:text-blue-300 font-medium">Register as Customer →</Link></p>
          </div>
        </div>
      </div>
    </div>
